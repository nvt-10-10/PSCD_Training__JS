...
function generateRandomData() {
  ...
  const randomRenderingEngine = renderingEngines[Math.floor(Math.random() * renderingEngines.length)];
...
function initData(entriesNumber, page, keyword, propertySort, order) {
  ...
  let n = page * entriesNumber + entriesNumber > arr.length ? arr.length : i + entriesNumber;
  for (i; i < n; i++) {
...
function initPage(arr, entriesNumber, page) {
  ...
  for (i = 0; i < pageNumber; i++) {
    let pageIndex = i + 1;
    html += i == page ? ` <li class="page__item page__active" data-page=${i}>${pageIndex}</li>` : ` <li class="page__item " data-page=${i}>${pageIndex}</li>`;
  }
  if (arr.length > entriesNumber) {
    $(".page").css("display", "flex");
    $(".page__list").html(html);
    $(".dataTables_info").html(`Showing ${entriesNumber * page + 1} to ${entriesNumber * (page + 1)} of ${arr.length} entries`);
  } else {
...
function search(keyword) {
  if (keyword.trim().length === 0) return arrInit;
    ...
    arrInit.forEach((element) => {
      if (["RenderingEngine", "Browser", "Platform", "EngineVersion", "CssGrade"].some((property) => element[property].toLowerCase().includes(keyword))) arrSearch.push(element);
    });
  }
  return arrSearch;
}
...
$(".table").on("click", ".table-title", function () {
  ...
  let newSort = sort.endsWith("__ASC") ? sort.replace("__ASC", "__DESC") : sort.replace("__DESC", "__ASC");
  ...
  $(this).hasClass("reverse") ? $(this).removeClass("reverse") : $(this).addClass("reverse");
});

$(".prev").on("click", function (event) {
  page = parseInt($(".page__active").data("page"));

  page - 1 < 0 ? event.stopPropagation() : initData(entriesNumber, --page, keyword, propertySort, order);
});

$(".next").on("click", function () {
  page = parseInt($(".page__active").data("page"));
  page + 1 == pageNumber ? event.stopPropagation() : initData(entriesNumber, ++page, keyword, propertySort, order);
});

$(".next").hover(function () {
  parseInt($(".page__active").data("page")) + 1 == pageNumber ? $(this).css("cursor", "no-drop") : $(this).css("cursor", "pointer");
});

$(".prev").hover(function () {
  parseInt($(".page__active").data("page")) == 0 ? $(this).css("cursor", "no-drop") : $(this).css("cursor", "pointer");
});
